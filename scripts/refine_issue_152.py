...
>(
    ...
    // Persistent read-only credential management is not implemented yet and
    // perCredMgmtRO is therefore absent from GetInfo.
...
legacy_replacement = '''    let protocol: u8 = parser.get(req_keys::PIN_UV_AUTH_PROTOCOL)?;
    ...
    // The superseded getPinToken command does not accept explicit
    // permissions or an RP ID.
...
# 0x40 is a defined CTAP 2.3 permission (pcmr), unsupported by this
# authenticator and therefore unauthorized rather than malformed.
text = text.replace(
...
# Replace the final pcmr expectation in the PIN-path test as well.
