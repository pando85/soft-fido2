...
fn handle_set_pin<C: AuthenticatorCallbacks, K: crate::key_provider::CredentialKeyProvider>(
    auth: &mut Authenticator<C, K>,
    parser: &MapParser,
) -> Result<Vec<u8>> {
    if auth.is_pin_set() {
...
fn handle_change_pin<C: AuthenticatorCallbacks, K: crate::key_provider::CredentialKeyProvider>(
    auth: &mut Authenticator<C, K>,
    parser: &MapParser,
) -> Result<Vec<u8>> {
    if !auth.is_pin_set() {
