...
use crate::{
    ...
    credential_key::{CredentialKey, CredentialKeyProvider},
...
pub fn handle<C: AuthenticatorCallbacks, K: CredentialKeyProvider>(
    ...
    let cred_data = AttestationCredential {
        id: credential_id,
        public_key: generated_key.cose_public_key,
        algorithm: alg,
    };
    ...
    let signature = auth
        .key_provider()
        .sign(&generated_key.key, alg, &sig_data)
        .map_err(StatusCode::from)?;
...
fn create_credential<C: AuthenticatorCallbacks, K: CredentialKeyProvider>(
    ...
    if options.rk || auth.config().force_resident_keys {
        ...
        auth.wrap_credential_with_key(credential_key, &rp.id, algorithm)
    }
}
