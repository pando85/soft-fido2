...
use crate::{
    ...
    credential_key::CredentialKeyProvider,
...
pub fn handle<C: AuthenticatorCallbacks, K: CredentialKeyProvider>(
    ...
    let mut credentials = if let Some(ref allow_list) = allow_list {
        ...
        for desc in allow_list {
            ...
            if let Ok((credential_key, cred_rp_id, algorithm)) = auth.unwrap_credential(&desc.id)
                && cred_rp_id == rp_id
            {
    ...
    let signature = auth
        .key_provider()
        .sign(
            &selected_cred.credential_key,
            selected_cred.algorithm,
            &sig_data,
        )
