...
fn test_channel_manager_basic() {
    ...
    let cmd = Cmd::Ping;
    let data = b"test";
    let packets = Packet::new_init(cid, cmd, data, None).expect("Failed to create packet");
...
fn test_channel_manager_multi_packet() {
    ...
    let packets = Packet::new_init(cid, cmd, &data, None).expect("Failed to create packets");
...
fn test_handler_ping_command() {
    ...
    // Create PING request
    let packets = Packet::new_init(cid, Cmd::Ping, data, None).expect("Failed to create packet");
    ...
    let response_message =
        Message::from_packets(&response_packets, None).expect("Failed to reassemble");
...
fn test_handler_init_command() {
    ...
    // Create INIT request
    let packets =
        Packet::new_init(broadcast_cid, Cmd::Init, &nonce, None).expect("Failed to create packet");
...
fn test_handler_cbor_command() {
    ...
    // Create CBOR request
    let packets = Packet::new_init(cid, Cmd::Cbor, &data, None).expect("Failed to create packet");
...
fn test_full_stack_message_round_trip() {
    ...
    let init_packets =
        Packet::new_init(broadcast_cid, Cmd::Init, &nonce, None).expect("Failed to create INIT");
    let init_response = ctaphid_handler
        .process_packet(init_packets[0].clone())
        .expect("Failed to process INIT");

    let init_message = Message::from_packets(&init_response, None).expect("Failed to parse INIT");
    let allocated_cid = u32::from_be_bytes([
        init_message.data[8],
        init_message.data[9],
    ...
    let ping_packets =
        Packet::new_init(allocated_cid, Cmd::Ping, ping_data, None).expect("Failed to create PING");
    let ping_response = ctaphid_handler
        .process_packet(ping_packets[0].clone())
        .expect("Failed to process PING");

    let ping_message = Message::from_packets(&ping_response, None).expect("Failed to parse PING");
    ...
    let cbor_packets = Packet::new_init(allocated_cid, Cmd::Cbor, &cbor_data, None)
    ...
    let cbor_message = Message::from_packets(&cbor_response, None).expect("Failed to parse CBOR");
...
fn test_multiple_channels() {
    ...
    let init1 =
        Packet::new_init(0xFFFFFFFF, Cmd::Init, &nonce1, None).expect("Failed to create INIT 1");
    let response1 = ctaphid_handler
        .process_packet(init1[0].clone())
        .expect("Failed to process INIT 1");
    let msg1 = Message::from_packets(&response1, None).expect("Failed to parse INIT 1");
    let cid1 = u32::from_be_bytes([msg1.data[8], msg1.data[9], msg1.data[10], msg1.data[11]]);

    // Initialize channel 2
    let init2 =
        Packet::new_init(0xFFFFFFFF, Cmd::Init, &nonce2, None).expect("Failed to create INIT 2");
    let response2 = ctaphid_handler
        .process_packet(init2[0].clone())
        .expect("Failed to process INIT 2");
    let msg2 = Message::from_packets(&response2, None).expect("Failed to parse INIT 2");
    ...
    let ping1 = Packet::new_init(cid1, Cmd::Ping, data1, None).expect("Failed to create PING 1");
    let ping2 = Packet::new_init(cid2, Cmd::Ping, data2, None).expect("Failed to create PING 2");
    ...
    let msg1 = Message::from_packets(&resp1, None).expect("Failed to parse response 1");
    let msg2 = Message::from_packets(&resp2, None).expect("Failed to parse response 2");
